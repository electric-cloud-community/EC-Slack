...
Use this method to create a new FlowPlugin::ECSlackRESTClient, e.g.

    my $client = FlowPlugin::ECSlackRESTClient->new($endpoint,
...
sub SendRealtimeMessage {
    ...
    $self->{method} = 'SendRealtimeMessage';
    $self->{methodParameters} = \%params;

    my $uri = "/chat.postMessage";
...
sub CreateChannel {
    ...
    $self->{method} = 'CreateChannel';
    $self->{methodParameters} = \%params;

    my $uri = "/conversations.create";
    logDebug "URI Template: $uri";
    $uri = renderOneLineTemplate($uri, %params);
    logDebug "Rendered URI: $uri";
